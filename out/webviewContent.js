...
<head>
    ...
    <style>
        * {
        ...
        .json-editor .json-highlighted .json-string { color: #ce9178; }
        .json-editor .json-highlighted .json-number { color: #b5cea8; }
        .json-editor .json-highlighted .json-boolean { color: #569cd6; }
        .json-editor .json-highlighted .json-null { color: #569cd6; }
        .json-editor .json-highlighted .json-key { color: #9cdcfe; }
        .json-editor .json-highlighted .json-punctuation { color: #d4d4d4; }
...
<body>
            ...
            <div id="editorTab" class="tab-content">
                ...
                <div class="form-group">
                    ...
                    <div class="json-editor">
                        <div class="json-highlighted" id="responseBodyHighlighted"></div>
                        <textarea id="responseBodyInput">{"message": "success"}</textarea>
                        <button class="json-toggle" id="responseBodyToggle" title="Toggle syntax highlighting">JSON</button>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="json-editor">
                        <div class="json-highlighted" id="responseHeadersHighlighted"></div>
                        <textarea id="responseHeadersInput">{"Content-Type": "application/json"}</textarea>
                        <button class="json-toggle" id="responseHeadersToggle" title="Toggle syntax highlighting">JSON</button>
                    </div>
                </div>
    ...
    <script>
        ...
        document.getElementById('clearBtn').addEventListener('click', () => {
            ...
            document.getElementById('responseBodyInput').dispatchEvent(new Event('input'));
            document.getElementById('responseHeadersInput').dispatchEvent(new Event('input'));
        });
        ...
        window.addEventListener('message', event => {
            ...
            if (message.command === 'loadRequest') {
                ...
                // Safely set response body
                const responseBodyInput = document.getElementById('responseBodyInput');
                responseBodyInput.value = req.responseBody || '{"message": "success"}';
                
                // Safely set response headers
                const responseHeadersInput = document.getElementById('responseHeadersInput');
                try {
                    responseHeadersInput.value = JSON.stringify(req.responseHeaders || {"Content-Type": "application/json"}, null, 2);
                } catch (e) {
                    responseHeadersInput.value = '{"Content-Type": "application/json"}';
                }
                ...
                // Trigger highlighting update after a short delay to ensure DOM is ready
                setTimeout(() => {
                    responseBodyInput.dispatchEvent(new Event('input'));
                    responseHeadersInput.dispatchEvent(new Event('input'));
                }, 100);
            }
        });

        // JSON Syntax Highlighting
        function applySyntaxHighlighting(text, element) {
            ...
            try {
                ...
                const frag = createHighlightedFragment(formatted);
                element.appendChild(frag);
            } catch (e) {
                // For invalid JSON, fall back to plain text (safe via textContent)
                element.textContent = text;
            }
        }

        function createHighlightedFragment(json) {
            ...
            while ((m = re.exec(json)) !== null) {
                if (m.index > lastIndex) {
                    fragment.appendChild(document.createTextNode(json.slice(lastIndex, m.index)));
                }
        ...
        function setupJSONEditor(textareaId, highlightedId, toggleId) {
            const textarea = document.getElementById(textareaId);
            const highlighted = document.getElementById(highlightedId);
            const toggle = document.getElementById(toggleId);
            const jsonEditor = textarea.parentElement;
            ...
            function updateHeight() {
                ...
                const newHeight = Math.max(scrollHeight, 100); // Minimum 100px
                
                textarea.style.height = newHeight + 'px';
                highlighted.style.height = newHeight + 'px';
                jsonEditor.style.height = newHeight + 'px';
            }

            function updateHighlighting() {
                if (highlightingEnabled) {
                    applySyntaxHighlighting(textarea.value, highlighted);
                    highlighted.style.display = 'block';
                    // keep textarea text transparent so highlighted layer shows
                    textarea.style.color = 'transparent';
                    toggle.classList.add('active');
                } else {
                    // hide highlighted layer and show real textarea text
        ...
        // Setup JSON editors
        setupJSONEditor('responseBodyInput', 'responseBodyHighlighted', 'responseBodyToggle');
        setupJSONEditor('responseHeadersInput', 'responseHeadersHighlighted', 'responseHeadersToggle');
    </script>
</body>
</html>`;
}
