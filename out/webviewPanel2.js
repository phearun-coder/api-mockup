...
class MockupWebviewPanel {
    constructor(panel, extensionUri, server) {
        this._panel = panel;
        ...
        this._panel.webview.onDidReceiveMessage(async (message) => {
            switch (message.command) {
                ...
                case 'stopServer':
                    ...
                    break;
                case 'addRequest':
                    this._server.addMockRequest(message.request);
                    this._postRequests();
                    break;
                case 'updateRequest':
                    this._server.updateMockRequest(message.id, message.request);
                    this._postRequests();
                    break;
                case 'removeRequest':
    ...
    static createOrShow(extensionUri, server) {
        ...
        const panel = vscode.window.createWebviewPanel(MockupWebviewPanel.viewType, 'API Mockup', column || vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    _update() {
        this._panel.webview.html = (0, webviewContent_1.getWebviewContent)(this._extensionUri);
    }
    _postServerStatus() {
